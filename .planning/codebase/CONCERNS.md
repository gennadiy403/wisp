...
- Issue: All 1900+ lines of application logic live in one file — `wisp.py`. Audio recording, transcription, HUD rendering, hotkey interception, note classification, merge logic, CLI subcommands, and onboarding are all interleaved.
- Files: `wisp.py`
...
- Fix approach: Extract into modules — `wisp/audio.py`, `wisp/hud.py`, `wisp/keys.py`, `wisp/notes/`, `wisp/cli.py`.

**Module-level code runs at import time:**
- Issue: `cli_main()` (line 562), `CONFIG = load_config()` (line 165), `PLUGINS = load_plugins()` (line 166), and OpenAI client initialization (line 583) all execute unconditionally at module load. This means even `wisp setup` initializes the OpenAI client and exits if `OPENAI_API_KEY` is missing.
- Files: `wisp.py` lines 165–583
- Impact: Any CLI subcommand that does not need the API key (e.g., `wisp plugin list`) still triggers `sys.exit(1)` if the env var is absent. Makes unit testing impossible without mocking or env setup.
...
- Files: `wisp.py` lines 1054–1120 and 1284–1336
...
- Files: `wisp.py` lines 724–726 and 828–838
...
- Issue: Line 1660 constructs a shell preview command using Python's `os.environ["WISP_PATHS"]` with embedded f-string quoting that will break for file paths containing spaces or special characters. The `{}` in the fzf `--preview` argument is also used as both an fzf placeholder and a Python format specifier.
- Files: `wisp.py` lines 1656–1672
- Trigger: Any note path containing a space (e.g., `~/wisp-notes/2026/04` is fine, but user-defined `output_dir` with spaces would break it).

## Security Considerations

**API keys stored in a plain-text shell script:**
- Risk: `~/.config/wisp/env` is a `source`-able shell file with `export KEY=value` lines. If the user's home directory has broad permissions, or a malicious process reads `~/.config/wisp/env`, both OpenAI and Anthropic keys are exposed in plaintext.
- Files: `wisp` (line 4), `wisp.py` (lines 349–366)
...
**`WISP_PATHS` env var in subprocess is unbounded:**
- Risk: `_fzf_pick()` passes all note paths as a newline-delimited env var `WISP_PATHS` (line 1667). If the index grows large, this may hit OS env var size limits (typically 128KB–2MB depending on macOS version).
- Files: `wisp.py` lines 1659–1667
...
- Risk: `soundfile` appears in `requirements.txt` but is not imported anywhere in `wisp.py`. It is an unnecessary dependency that inflates the install footprint and introduces a potential vulnerability surface.
...
- Files: `wisp.py` lines 154, 388, 472, 1346
...
- Files: `wisp.py` lines 782–793, 1029–1044, 1181–1206

**`wisp notes` edit confirmation has no diff review for non-TTY invocation:**
...
- What's not tested: Every function in `wisp.py` — audio encoding, hallucination filtering, note classification, merge decision, frontmatter parsing, slug sanitization, YAML loading with fallback, CLI routing.
- Files: `wisp.py` (entire file)
