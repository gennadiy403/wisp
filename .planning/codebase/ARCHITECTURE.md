...
**Key Characteristics:**
- All application logic lives in one file: `wisp.py` (1912 lines)
...
- Location: `wisp.py`, functions `cli_main()`, `cli_setup()`, `cli_plugin()`, `cli_notes()`
...
- Purpose: Load `~/.config/wisp/config.yaml` and discover plugins from `~/.config/wisp/plugins/`
- Location: `wisp.py`, functions `load_config()`, `load_plugins()`, `build_whisper_prompt()`, `build_notes_config()`
- Contains: YAML loading with JSON fallback, plugin metadata merging
- Depends on: Filesystem (`~/.config/wisp/`)
...
- Location: `wisp.py`, functions `start_recording()`, `stop_and_transcribe()`, `cancel_recording()`, `audio_callback()`
...
- Location: `wisp.py`, function `_encode_and_transcribe()`
...
- Location: `wisp.py`, `stop_and_transcribe()` branching on `note_mode` / `predict_mode`
- Three paths:
...
- Location: `wisp.py`, functions `classify_note()`, `save_or_merge_note()`, `save_as_note()`, `_save_note_with_meta()`, `_decide_merge()`, `_confirm_merge()`, `_apply_merge_append()`
...
- Location: `wisp.py`, functions `generate_continuations()`, `show_predict_menu()`, class `PredictController`
...
- Location: `wisp.py`, functions `setup_hud()`, `set_hud()`
...
- Location: `wisp.py`, functions `install_monitor()`, `cg_event_callback()`
...
- Location: `wisp.py`, functions `paste_text()`, `_press_enter()`
...
- Examples: `~/.config/wisp/plugins/notes/plugin.yaml`, `examples/notes/plugin.yaml`
- Pattern: Directory under `~/.config/wisp/plugins/<name>/` with `plugin.yaml` (required), `contexts.yaml` (optional), `stuck.yaml` (optional). Host reads and merges at startup — no executable code in plugins.
...
**Note index (JSONL):**
- Purpose: Lightweight searchable record of all saved notes for merge-check and `wisp notes` picker
- Location: `~/wisp-notes/index/recent.jsonl` (configurable via `NOTES_CFG["index_file"]`)
...
- Location: `~/wisp-notes/{year}/{month}/{date}_{time}_{slug}.md`
...
**Daemon startup:**
- Location: `wisp.py`, `__main__` block (line 1882)
- Triggers: `python3 wisp.py` (or `./wisp` shell script)
...
- Location: `wisp.py`, `cli_main()` (line 519) — runs at module load before `__main__`
- Routes: `setup`, `plugin list|init|remove`, `notes`, `note <text>`

**Note text CLI:**
- Location: `wisp.py`, `__main__` block, `_NOTE_CLI_TEXT` branch (line 1886)
- Triggers: `wisp note <text>` or `echo text | wisp note`
...
**Logging:** `print(..., flush=True)` throughout — no structured logging framework. All output goes to stdout of the terminal running wisp.
...
**Authentication:** API keys loaded from environment variables (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`). Keys sourced from `~/.config/wisp/env` (shell export file, not dotenv). Setup wizard writes this file at `chmod 600`.
