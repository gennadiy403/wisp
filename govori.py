...
def clean_transcription(text):
    ...
    system = f"""Ты чинишь очевидные ошибки Whisper в русскоязычной транскрипции.

ТВОЙ ПРИНЦИП: минимальное вмешательство. Если сомневаешься — не трогай.

ИСПРАВЛЯЙ ТОЛЬКО:
1. Неправильно распознанные имена и термины из словаря (см. ниже):
   "MarkVisa"/"маркиз" → "Marquiz", "Трайл-Март" → "Travelmart",
   "БИОС" → "VPS" (если по контексту речь о сервере), и т.д.
2. Очевидно слипшиеся короткие слова: "Наташи не" должно быть
   "у Наташи надо" если по контексту ясно.
3. Очевидно перепутанные близкие фонемы: "взять"↔"учесть",
   "не" вместо "надо", и т.п. — только когда исходный текст
   семантически не имеет смысла, а альтернатива явно подходит.
   ИЗВЕСТНЫЕ частые ошибки Whisper, правь автоматически:
   - "софтра" → "завтра"
   - "Наташи не взять" / "Наташи невзять" → "у Наташи взять" или "у Наташи надо взять"
   - "MarkVisa" / "Маркиз" / "Марк Виз" → "Marquiz"
   - "Трайл-Март" / "Трейл-Март" / "Трейл-Мартом" → "Travelmart"
   - "BIOS настроить" (если по контексту о сервере) → "VPS настроить"
4. Предлог при проекте: "к Marquiz" → "по Marquiz"/"в Marquiz"
   (когда речь о задачах/работе в проекте, не о физическом движении).
5. Пунктуацию — точки в конце предложений, запятые при перечислениях.

ЗАПРЕЩЕНО (важно!):
- Удалять любые слова, даже "эээ", "ну", "так", "вот", "сейчас", "короче" —
  ОСТАВЛЯЙ ИХ! Пользователь хочет сохранить свою речь как есть.
- Удалять повторы, запинки, самоперебивания — ОСТАВЛЯЙ.
- Удалять meta-комментарии типа "я хочу сделать заметку" — ОСТАВЛЯЙ.
- Переформулировать, перефразировать, сокращать, улучшать стиль.
- Менять порядок слов или предложений.
- Добавлять факты, выводы, интерпретации.
- Расшифровывать аббревиатуры.
- Переводить на другой язык.
- Менять регистр (я/ты/мы), лексику, разговорный тон.

ПРАВИЛО: если исходный текст читается нормально — верни его БЕЗ ИЗМЕНЕНИЙ.
Вмешивайся только когда видишь явную ошибку распознавания.
...
Верни ТОЛЬКО текст — без объяснений, кавычек, markdown."""
    ...
    try:
        ...
        # Sanity check: reject если текст укорочен более чем на 20% —
        # cleanup теперь только чинит точечные ошибки, не удаляет контент.
        # Если Haiku сократил — скорее всего он убирал паразиты/повторы/meta,
        # что теперь запрещено. Возвращаем оригинал.
        if len(cleaned) < len(text) * 0.8 or len(cleaned) > len(text) * 1.3:
